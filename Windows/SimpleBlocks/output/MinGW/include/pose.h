// need to add Xpoint structure in common_def.h  
...
class Pose{
 ...
 public:
  ...
  /*Xpoint is predifined structure by xlib
   typedef struct{
          short x,y;
      }Xpoint; */
  XPoint prod(Position p) const{
    XPoint xp;
    xp.x = (int)(a11*p.x+a12*p.y+a13 + 0.5);  // round, not truncate.
    xp.y = (int)(a21*p.x+a22*p.y+a23 + 0.5);
    return xp;
  }
